# Adapted from https://github.com/Graph-COM/HEPT (MIT License), example/transformer.py
# Only change from upstream: relative imports so this lives inside cat_finder's package.
import torch
from torch import nn
from torch_geometric.nn import MLP
from einops import rearrange

from .hept_attention import HEPTAttention
...
class Transformer(nn.Module):
    """Note: coords[:, :2] are treated as (eta, phi)-like local coordinates for
    region partitioning — for CDC hits this is (cdchit_middle_x, cdchit_middle_y)."""
    ...
    def forward(self, x, coords, batch):
        """
        x:      (N, in_dim)   all hit features for the whole minibatch
        coords: (N, coords_dim)  first 2 dims used for LSH region partitioning
        batch:  (N,)           event index per hit, same as GravNetConv's `batch`
        """
...
class Attn(nn.Module):
    def __init__(self, coords_dim, **kwargs):
        ...
        # +coords_dim for the position channel appended inside HEPTAttention
    ...
    def forward(self, x, kwargs):
        ...
        # note: kwargs already contains "coords" (set in prepare_input); HEPTAttention
        # reads it from there. w_rpe is passed explicitly since it's an Attn-owned module.
